...
public class Program220 {
    ...
    public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
    		...
    		for (int i = 0; i < nums.length; i++) {
    				long num = nums[i];
    				Long floor = tree.floor((long) (num+t));
    				Long ceiling = tree.ceiling((long) (num-t));
    				if((floor!=null&&floor>=num)||(ceiling!=null&&ceiling<=num))return true;
    ...
    /*
     *s a followup question, it naturally also requires maintaining a window of size k. 
	 ...
	 *it reminds us of doing a range check which is implemented in tree data structure and would take O(LogN) if a balanced tree structure is used, or doing a bucket check which is constant time. We shall just discuss the idea using bucket here.
	 *Bucketing means we map a range of values to the a bucket. For example,
	 *if the bucket size is 3, we consider 0, 1, 2 all map to the same bucket. 
